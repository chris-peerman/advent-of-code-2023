...
const parseLine = (line) => /([A-Z0-9]+) (\d*)/.exec(line).slice(1, 3);

const sortCardsInRow = (data) => [[...data[0]].sort((a, b) => LOOKUP[a] < LOOKUP[b] ? 1 : -1).join(''), data[1]];

const getNumberOfPairs = (cards) => {
    ...
    switch (Object.keys(pairs).length) {
        case 1: return 6;
        case 2: return [...Object.values(pairs)].sort((a, b) => a < b ? 1 : -1)[0] === 4 ? 5 : 4;
        case 3: return [...Object.values(pairs)].sort((a, b) => a < b ? 1 : -1)[0] === 3 ? 3 : 2;
        case 4: return 1;
        case 5: return 0;
    }
};
...
const sortRows = (a, b) => {
    const numA = getNumberOfPairs(a[0]);
    const numB = getNumberOfPairs(b[0]);

    if (numA === numB) {
        for(let i=0; i < Math.min(a[0].length, b[0].length); i++) {
            let aScore = LOOKUP[a[0].charAt(i)];
            let bScore = LOOKUP[b[0].charAt(i)];
...
const getSortedRows = (input) => input.map(parseLine).sort(sortRows);
