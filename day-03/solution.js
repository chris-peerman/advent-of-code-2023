...
const partNumbers = SCHEMATIC.reduce((acc, line, row, array) => {
    const searchStart = (row > 0 ? (row - 1) : row);
    const searchEnd = (row < array.length - 1) ? row + 1 : row;
    
    const matches = [...line.matchAll(/(\d+)/g)];
    matches.forEach((match) => {
        const num = match[1];
        let startIndex = match.index - 1;
        let endIndex = match.index + num.length + 1;

        if (startIndex < 0) startIndex = 0;
        if (endIndex >= line.length) endIndex = line.length - 1;

        for(let i=searchStart; i <= searchEnd; i++) {
            const symbols = array[i].substring(startIndex, endIndex).replaceAll(/[\d.]/g, '');
            if (symbols.length > 0) {
                acc.push(num);
                break;
            }
...
console.log(partNumbers.map(num => parseInt(num, 10)).reduce((acc, part) => acc + part));